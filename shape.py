...
class Shape(ABC):
    ...
    def decelerate(self, deceleration):
       ...
       # while self.pull.x  > -1:
            ...
            print("debug:{}, {}".format(self.pull.x, self.pull.y))
            self.pull.x -= (deceleration * math.cos(math.radians(self.rotation)))
            self.pull.y -= (deceleration * math.sin(math.radians(self.rotation)))
